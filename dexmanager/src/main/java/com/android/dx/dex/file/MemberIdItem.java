...
public abstract class MemberIdItem extends IdItem {
    ...
    public final void writeTo(DexFile file, AnnotatedOutput out) {
        ...
        int typoidIdx = getTypeidIdx(file);

        if (out.annotates()) {
            out.annotate(0, indexString() + ' ' + cst.toHuman());
            out.annotate(2, "  class_idx: " + Hex.u2(classIdx));
            out.annotate(2, String.format("  %-10s %s", getTypeidName() + ':',
                            Hex.u2(typoidIdx)));
     ...
     */
    protected abstract int getTypeidIdx(DexFile file);
     ...
     */
    protected abstract String getTypeidName();
