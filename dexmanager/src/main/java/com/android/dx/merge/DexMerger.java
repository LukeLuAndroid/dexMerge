...
public final class DexMerger {
    ...
    public DexBuffer merge() throws IOException {
        ...
        if (wastedByteCount > +compactWasteThreshold) {
            DexMerger compacter = new DexMerger(dexOut, new DexBuffer(), CollisionPolicy.FAIL, compactedSizes);
        ...
        System.out.printf("Merged dex A (%d defs/%.1fKiB) with dex B " + "(%d defs/%.1fKiB). Result is %d defs/%.1fKiB. Took %.1fs%n", dexA.getTableOfContents().classDefs.size,
                dexA.getLength() / 1024f, dexB.getTableOfContents().classDefs.size, dexB.getLength() / 1024f, result.getTableOfContents().classDefs.size,
    ...
    abstract class IdMerger<T extends Comparable<T>> {
        ...
        private final List<T> writedString = new ArrayList<>();
        ...
        public final void mergeSorted() {
            ...
            while (true) {
                ...
                if (toWrite instanceof String && writedString.contains(toWrite)) {
                    continue;
                }
                write(toWrite);
                outCount++;
    ...
    private void mergeStringIds() {
        new IdMerger<String>(idsDefsOut) {
            ...
            String read(DexBuffer.Section in, IndexMap indexMap, int index) {
                return in.getHost().sortStrings(false).get(index);
                // return in.readString();
            }

            @Override
            void updateIndex(int offset, IndexMap indexMap, int oldIndex, int newIndex) {
                Map<Integer, List<Integer>> map = indexMap.getHost().indexMapping();
                if (map != null && map.size() > 0) {
                    List<Integer> list = map.get(oldIndex);
                    for (Integer index : list) {
                        if (indexMap.stringIds[index] == 0) {
                            indexMap.stringIds[index] = newIndex;
                        }
                    }
                } else indexMap.stringIds[oldIndex] = newIndex;
            }
    ...
    private void mergeTypeIds() {
        new IdMerger<Integer>(idsDefsOut) {
            ...
            Integer read(DexBuffer.Section in, IndexMap indexMap, int index) {
                return in.getHost().sortTypeIds(indexMap, false).get(index);
...
//                List<Integer> map = indexMap.getHost().typeIdMapping();
//                if (map != null && map.size() > 0)
//                    indexMap.typeIds[map.get(oldIndex)] = (short) newIndex;
//                else indexMap.typeIds[oldIndex] = (short) newIndex;
                Map<Integer, List<Integer>> map = indexMap.getHost().typeIdMapping();
                if (map != null && map.size() > 0) {
                    List<Integer> list = map.get(oldIndex);
                    for (Integer index : list) {
                        if (indexMap.typeIds[index] == 0) {
                            indexMap.typeIds[index] = (short) newIndex;
                        }
                    }
                } else {
    ...
    private void mergeFieldIds() {
        new IdMerger<FieldId>(idsDefsOut) {
            ...
            FieldId read(DexBuffer.Section in, IndexMap indexMap, int index) {
                return in.getHost().sortFieldIds(indexMap, false).get(index);
            ...
            void updateIndex(int offset, IndexMap indexMap, int oldIndex, int newIndex) {
                ...
                List<Integer> map = indexMap.getHost().fieldMapping();
                if (map != null && map.size() > 0)
                    indexMap.fieldIds[map.get(oldIndex)] = (short) newIndex;
                else indexMap.fieldIds[oldIndex] = (short) newIndex;
            }
    ...
    private void mergeMethodIds() {
        new IdMerger<MethodId>(idsDefsOut) {
            ...
            MethodId read(DexBuffer.Section in, IndexMap indexMap, int index) {
                return indexMap.getHost().sortMethodIds(indexMap, false).get(index);
            ...
            void updateIndex(int offset, IndexMap indexMap, int oldIndex, int newIndex) {
                ...
                List<Integer> map = indexMap.getHost().methodMapping();
                if (map != null && map.size() > 0)
                    indexMap.methodIds[map.get(oldIndex)] = (short) newIndex;
                else indexMap.methodIds[oldIndex] = (short) newIndex;
            }
     ...
     * <p>
     * TODO: this may write multiple copies of the same annotation set. We
     ...
     */
    private void transformAnnotationDirectory(DexBuffer.Section directoryIn, IndexMap indexMap) {
    ...
    private static class WriterSizes {
        ...
        public void plus(TableOfContents contents, boolean exact) {
            idsDefs += contents.stringIds.size * SizeOf.STRING_ID_ITEM + contents.typeIds.size * SizeOf.TYPE_ID_ITEM + contents.protoIds.size * SizeOf.PROTO_ID_ITEM
                    + contents.fieldIds.size * SizeOf.MEMBER_ID_ITEM + contents.methodIds.size * SizeOf.MEMBER_ID_ITEM + contents.classDefs.size * SizeOf.CLASS_DEF_ITEM;
        ...
        public int size() {
            return header + idsDefs + mapList + typeList + classData + code + stringData + debugInfo + encodedArray + annotationsDirectory + annotationsSet + annotationsSetRefList
                    + annotation;
        }
    }
